...
def nll_fn2(X, y):
    ...
    def step(theta):
        ...
        for train_index, test_index in loo.split(X):
            ...
            y_model_test = slope * X_test[:, 2] + zp + s3 * X_test[:, 3] + s4*X_test[:, 4] + s5*X_test[:, 5]
...
if __name__ == '__main__':
    ...
    for dt_col in dt_cols:
        ...
        try:
            ...
            X = df[["LON", "LAT", "ELEV.m.", "coastDist", "lai", "albedo"]].values
            y = df['T'].values
            ...
            if len(data_df) > 1:
                ...
                if pos.success:
                    ...
                    pd.DataFrame.from_dict(Hyper).to_csv(
                        'KF5_Tmin_Elev_CDI_LAI_ALBEDO_'+iCODE+'_hyper.csv', sep=',', index=False)
