...
#         for train_index, test_index in kf.split(X):
        for train_index, test_index in loo.split(X):
            ...
            y_model = slope * X_train[:, 2] + zp + s3*X_train[:,3] + s4*X_train[:,4]
            ind, = np.where(X_train[:, 2] > 2150)
            y_model[ind] += s2 * (X_train[:, 2][ind] - 2150)

            y_model_test = slope * X_test[:, 2] + zp + s3*X_train[:,3] + s4*X_train[:,4]
...
if __name__ == '__main__':
    ...
    for dt_col in dt_cols:
        ...
        try:
            ...
            X = df[["LON", "LAT", "ELEV.m.", "RF", "windSpeed"]].values
            y = df['T'].values
            ...
            if len(data_df) > 1:
                ...
                y_model = slope * X[:, 2] + zp + s3*X[:,3] + s4*X[:,4]
                ...
                if pos.success:
                    ...
                    pd.DataFrame.from_dict(Hyper).to_csv(
                        'Tmax_Elev_RF_WS_'+ISLAND_code+'_hyper.csv', sep=',', index=False)
