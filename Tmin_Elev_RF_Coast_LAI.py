...
#         for train_index, test_index in kf.split(X):
        for train_index, test_index in loo.split(X):
            ...
            y_model = slope * X_train[:, 2] + zp + s3*X_train[:,3] + s4*X_train[:,4] + s5*X_train[:,5]
            ind, = np.where(X_train[:, 2] > 2150)
            y_model[ind] += s2 * (X_train[:, 2][ind] - 2150)

            y_model_test = slope * X_test[:, 2] + zp + s3*X_train[:,3] + s4*X_train[:,4] + s5*X_train[:,5]
...
if __name__ == '__main__':
    ...
    ISLAND_code = str(sys.argv[1])

    pr = pd.read_csv("tmin_predictors.csv",
                     encoding="ISO-8859-1", engine='python')
    pr = pr.set_index("SKN")
    pr = pr[(pr.Island == ISLAND_code)]
    ...
    rf2 = rf2[(rf2.Island == ISLAND_code)]
    ...
    df2 = df2[(df2.Island == ISLAND_code)]
    ...
    for dt_col in dt_cols:
        ...
        try:
            ...
            X = df[["LON", "LAT", "ELEV.m.", "RF", "coastDist", "lai"]].values
            y = df['T'].values
            ...
            if len(data_df) > 1:
                ...
                pos = minimize(nll_fn2(X, y), [
                               0, -0.002, 0, 0, 0, 0], method='L-BFGS-B')
                ...
                y_model = slope * X[:, 2] + zp + s3*X[:,3] + s4*X[:,4] + s5*X[:,5]
                ...
                if pos.success:
                    ...
                    pd.DataFrame.from_dict(Hyper).to_csv(
                        'Tmin_Elev_RF_Coast_LAI_'+ISLAND_code+'_hyper.csv', sep=',', index=False)
