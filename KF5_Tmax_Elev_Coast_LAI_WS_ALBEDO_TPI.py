...
def nll_fn2(X, y):
    ...
    def step(theta):
        ...
        for train_index, test_index in loo.split(X):
            ...
            y_model = slope * X_train[:, 2] + zp + s3 * X_train[:, 3] + s4*X_train[:, 4] + s5*X_train[:, 5] + s6*X_train[:, 6] + s7*X_train[:, 7]
            ind, = np.where(X_train[:, 2] > 2150)
            y_model[ind] += s2 * (X_train[:, 2][ind] - 2150)

            y_model_test = slope * X_test[:, 2] + zp + s3 * X_test[:, 3] + s4*X_test[:, 4] + s5*X_test[:, 5] + s6*X_test[:, 6] + s7*X_test[:, 7]
...
if __name__ == '__main__':
    ...
    for dt_col in dt_cols:
        ...
        try:
            ...
            X = df[["LON", "LAT", "ELEV.m.", "coastDist", "lai", "windSpeed", "albedo", "tpi"]].values
            y = df['T'].values
            ...
            if len(data_df) > 1:
                ...
                pos = minimize(nll_fn2(X, y), [
                               0, -0.002, 0, 0, 0, 0, 0, 0], method='L-BFGS-B')
                ...
                y_model = slope * X[:, 2] + zp + s3*X[:,3] + s4 * X[:,4] + s5 * X[:,5]+ s6 * X[:,6]+ s7 * X[:,7]
                ...
                if pos.success:
                    ...
                    Hyper['t7'].append(theta[7])

                    pd.DataFrame.from_dict(Hyper).to_csv(
                        'KF5_Tmax_Elev_CDI_LAI_WS_ALBEDO_TPI_'+iCODE+'_hyper.csv', sep=',', index=False)
