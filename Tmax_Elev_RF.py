...
#         for train_index, test_index in kf.split(X):
        if True: # for train_index, test_index in loo.split(X):
            X_train, X_test = X, X # X[train_index], X[test_index]
            ...
            y_model_test = slope * X_test[:, 2] + zp + s3*X_train[:,3]
            ...
            if len(data_df) > 1:
                ...
                if pos.success:
                    ...
                    pd.DataFrame.from_dict(Hyper).to_csv(
                        'Tmax_Elev0_RF0_'+ISLAND_code+'_hyper.csv', sep=',', index=False)
